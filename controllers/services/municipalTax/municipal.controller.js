...
import municipalTaxModel from "../../../model/services/municipalTax/municipalTax.model.js";
import fs from "fs";
...
export const GetMunicipalTaxOperatortList = async (req, res) => {
    try {

        const operators = Operators.filter((op) => {
            return op.Category === "Municipal Taxes"
        });
        ...
        return res.status(200).json(new APIResponse("Municipal Taxes Operator fetched successfully!", 200, operators));
    } catch (error) {
        return res.status(500).json(new APIError("Error: " + error.message, 500));
    }
};

export const MunicipalTaxOperatorConfig = async (req, res) => {
    try {
        const { billerId } = req.params;

        const operator = Operators.find(
            (op) => op.Category === "Municipal Taxes" && op.BillerId === billerId
        );

        if (!operator) {
            return res.status(400).json(new APIError("Invalid operator", 400));
        }
        return res.status(200).json(new APIResponse("Municipal Taxes Operator", 200, operator));
...
export const createMunicipalTaxPayment = async (req, res) => {
    try {
        ...
        let response = MunicipalTaxAPI(req.body);
