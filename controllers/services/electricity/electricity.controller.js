...
export const GetElectricityOptByBillerID = async (req, res) => {
    try {
       const billerId=req.params.billerId;
        ...
        const operator = await electricityModel.findOne({ "parameter.billerId": billerId });
...
export const GetElectricitCardOperator = async (req, res) => {
    try {
     const { operatorName,category,regex} = req.body;
        ...
        operator = await electricityModel.findOne({ "parameter.billerId": billerId });


        if (!operator) {
            operator = await electricityModel.create({
                userId:req.user.id,
                "parameter.opId":opId,
