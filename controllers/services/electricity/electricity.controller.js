...
import electricityModel from "../../../model/services/electricity/electricity.model.js";
import fs from "fs";
...
export const GetElectricityOperatortList = async (req, res) => {
    try {

        const operators = Operators.filter((op) => {
            return op.Category === "Electricity"
        });
        ...
        return res.status(200).json(new APIResponse("Electricity Operator fetched successfully!", 200, operators));
    } catch (error) {
        return res.status(500).json(new APIError("Error: " + error.message, 500));
    }
};
...
export const ElectricityOperatorConfig = async (req, res) => {
    try {
        const { billerId } = req.params;

        const operator = Operators.find(
            (op) => op.Category === "Electricity" && op.BillerId === billerId
        );
        ...
        return res.status(200).json(new APIResponse("Electricity Operator", 200, operator));
    } catch (error) {
...
export const createElectricityPayment = async (req, res) => {
    try {
        ...
        let response = ElectricityAPI(req.body);
