...
export const GetbroadbandOperator = async (req, res) => {
    try {
     const { operatorName,category,regex} = req.body;
        ...
        operator = await broadbandModel.findOne({ "parameter.billerId": billerId });
        ...
        return res.status(200).json(new APIResponse("BoradBand Operator fetched successfully!", 200, operator));
    } catch (error) {
        return res.status(500).json(new APIError("Error: " + error.message, 500));
    }
};
