...
import cableModel from "../../../model/services/cable/cable.model.js";
import fs from "fs";
...
export const GetCableOptByBillerID = async (req, res) => {
    try {
        ...
        return res.status(200).json(new APIResponse("Cable Operator fetched successfully!", 200, operator));
    } catch (error) {
        return res.status(500).json(new APIError("Error: " + error.message, 500));
    }
};

export const GetCableOperatortList = async (req, res) => {
    try {

        const operators = Operators.filter((op) => {
            return op.Category === "Cable"
        });
        ...
        return res.status(200).json(new APIResponse("Cable Operator fetched successfully!", 200, operators));
    } catch (error) {
        return res.status(500).json(new APIError("Error: " + error.message, 500));
    }
};

export const CableOperatorConfig = async (req, res) => {
    try {
        const { billerId } = req.params;

        const operator = Operators.find(
            (op) => op.Category === "Cable" && op.BillerId === billerId
        );

        if (!operator) {
            return res.status(400).json(new APIError("Invalid operator", 400));
        }
        return res.status(200).json(new APIResponse("Cable Operator", 200, operator));
    } catch (error) {
        return res.status(500).json(new APIError("Error: " + error.message, 500));
    }
};

export const ValidateCableOperator = async (req, res) => {
    try {
        const { billerId } = req.params;
        ...
        if (!hasInvalid) {
            await cableModel.create({
                userId: req.user.id,
                "parameter.category": "Cable",
...
export const createCablePayment = async (req, res) => {
    try {
        ...
        let response = CableAPI(req.body);
