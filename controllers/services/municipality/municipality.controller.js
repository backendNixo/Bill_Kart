...
import municipalityModel from "../../../model/services/municipality/municipality.model.js"
import fs from "fs";
import { OrderHistory } from "../../../model/users/orderHistory.model.js";
const Operators = JSON.parse(fs.readFileSync("./operators.json"));


export const GetMunicipalityOptByBillerID = async (req, res) => {
    try {
       const billerId = req.params.billerId;
        ...
        return res.status(200).json(new APIResponse("Municipality Operator fetched successfully!", 200, operator));
    } catch (error) {
        return res.status(500).json(new APIError("Error: " + error.message, 500));
    }
};

export const GetMunicipalityOperatortList = async (req, res) => {
    try {

        const operators = Operators.filter((op) => {
            return op.Category === "Municipality"
        });
...
export const MunicipalityOperatorConfig = async (req, res) => {
    try {
        const { billerId } = req.params;

        const operator = Operators.find(
            (op) => op.Category === "Municipality" && op.BillerId === billerId
        );
...
export const ValidateMunicipalitytOperator  = async (req, res) => {
    try {
        const { billerId } = req.params;
        ...
        if (!hasInvalid) {
            await municipalityModel.create({
                userId: req.user.id,
                "parameter.category": "Municipality",
...
export const createMunicipalityPayment = async (req, res) => {
    try {
        ...
        let response = MunicipalityAPI(req.body);
