...
export const GetLPGOptByBillerID = async (req, res) => {
    try {
        ...
        const operator = await LPGModel.findOne({ "parameter.billerId": billerId });

        return res.status(200).json(new APIResponse("LPG Booking Operator fetched successfully!", 200, operator));
    } catch (error) {
        return res.status(500).json(new APIError("Error: " + error.message, 500));
    }
};

export const GetLPGOperatortList = async (req, res) => {
    try {

        const operators = Operators.filter((op) => {
            return op.Category === "LPG Booking"
        });
        ...
        return res.status(200).json(new APIResponse("LPG Booking Operator fetched successfully!", 200, operators));
    } catch (error) {
        return res.status(500).json(new APIError("Error: " + error.message, 500));
    }
};

export const LPGOperatorConfig = async (req, res) => {
    try {
        const { billerId } = req.params;

        const operator = Operators.find(
            (op) => op.Category === "LPG Booking" && op.BillerId === billerId
        );
...
export const ValidateLPGtOperator = async (req, res) => {
    try {
        ...
        if (hasInvalid) {
            await LPGModel.create({
                userId: req.user.id,
                "parameter.category": "LPG Booking",
