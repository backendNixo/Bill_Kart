...
export const GetlandlineOptByBillerID = async (req, res) => {
    try {
       const billerId=req.params.billerId;
        ...
        const operator = await landlineModel.findOne({ "parameter.billerId": billerId });
...
export const GetlandlineOperator = async (req, res) => {
    try {
     const { operatorName,category,regex} = req.body;
        ...
        operator = await landlineModel.findOne({ "parameter.billerId": billerId });
