...
export const GetCreditCardOptByBillerID = async (req, res) => {
    try {
...
export const GetCreditCardOperator = async (req, res) => {
    try {
     const { operatorName,category,regex} = req.body;
        ...
        operator = await creditCardModel.findOne({ "parameter.billerId": billerId });


        if (!operator) {
            operator = await creditCardModel.create({
                userId:req.user.id,
                "parameter.opId":opId,
